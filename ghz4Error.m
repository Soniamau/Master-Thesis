...
X=fliplr(X); %Just reorganizing columns. Columns now correspond to qubits {1,9}, rows are the states
 
for j=1:1:16
    A=X;
    A(:,j)=mod(X(:,j)+1,2); %Introducimos errores. 1 error a cada paso, ie, volteamos una columna
H=[];
J=[];
K=[];
L=[];
